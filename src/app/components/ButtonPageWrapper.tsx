...
/**
 * Wrapper para a página Button (logada)
 * Configura o layout fixo com gradiente e scroll
 */
export function ButtonPageWrapper() {
  useEffect(() => {
    const setupButtonPage = () => {
      ...
      // Fix main container
      const mainContainer = document.querySelector('[data-name="button"]');
      if (mainContainer) {
      ...
      const header = document.querySelector('[data-name="button"] > div:first-child');
      if (header) {
      ...
      const mainContent = document.querySelector('[data-name="button"] > div:nth-child(2)');
      if (mainContent) {
      ...
      const leftSidebar = document.querySelector('[data-name="button"] > div:nth-child(2) > div:first-child');
      if (leftSidebar) {
      ...
      const centerContent = document.querySelector('[data-name="button"] > div:nth-child(2) > div:nth-child(2)');
      if (centerContent) {
      ...
      const rightSidebar = document.querySelector('[data-name="button"] > div:nth-child(2) > div:nth-child(3)');
      if (rightSidebar) {
    ...
    setupButtonPage();
    const timer1 = setTimeout(setupButtonPage, 100);
    const timer2 = setTimeout(setupButtonPage, 300);
    const timer3 = setTimeout(setupButtonPage, 500);
